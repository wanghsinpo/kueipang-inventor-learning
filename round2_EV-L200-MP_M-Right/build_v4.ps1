## Round 2 v4 — adds the 45° chord cut at SW corner (the missing plane 2 feature).
## From geometry dump: plane equation x + y = -45.21 (face normal NE: 0.71, 0.71, 0)
## So material is where x+y > -45.21; SW corner where x+y < -45.21 is REMOVED.
...
function Start-Nag9([uint32]$invPid) {
...
"@
        ...
        while ((Get-Date) -lt $stop) {
            ...
            [N9]::EnumWindows({param($h,$l)
                $sb = New-Object Text.StringBuilder 256
                [void][N9]::GetWindowText($h, $sb, 256)
                [uint32]$wp=0; [void][N9]::GetWindowThreadProcessId($h, [ref]$wp)
                if ($wp -eq $p -and [N9]::IsWindowVisible($h) -and ($sb.ToString() -match 'Configurator|Sign In|Welcome')) { $script:hits.Add($h) }
                $true
            }, [IntPtr]::Zero) | Out-Null
            foreach ($h in $hits) { [void][N9]::PostMessage($h, 0x10, [IntPtr]::Zero, [IntPtr]::Zero) }
...
# ---- Step 2: Ø52 internal cavity from top ----
...
# ---- Step 5: 2 Ø7.9 c'bores deep 5.5 on top ----
...
# ---- Step 6: 45° chord cut at NW corner (matches real TOP view: cut at upper-left)
# Triangle vertices cover NW quadrant where -x+y > 45.21:
Write-Host "Step 6: 45° chord cut at NW corner..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true
...
$sp = @()
foreach ($pt in $pts) {
    $sp += $s6.SketchPoints.Add($tg.CreatePoint2d((MM $pt[0]), (MM $pt[1])), $false)
}
for ($i = 0; $i -lt 3; $i++) {
    $null = $s6.SketchLines.AddByTwoPoints($sp[$i], $sp[($i + 1) % 3])
}
...
# ---- Step 7: small notch on bottom centered front (y<0 side) ----
# From geometry: 7mm wide (x=±3.5), short Y extent. Approx 7 × 5 × 3 deep.
# But to match the "hidden" planes 3,5,9,10 with y_root values that suggest
# 2 notches at ±Y, let me also add at top.
# Actually FRONT view shows ONE notch at bottom-center. Let me do ONE small notch.
Write-Host "Step 7: Small notch at bottom edge..." -ForegroundColor Cyan
$bottomFace = $null; $bestA = 0
foreach ($f in $body.Faces) {
    if ($f.SurfaceType -ne 5890) { continue }
    $r = $f.Geometry.RootPoint; $n = $f.Geometry.Normal
    if ([Math]::Abs($r.Z) -lt 0.01 -and [Math]::Abs($n.Z) -gt 0.5) {
        $a = $f.Evaluator.Area
        if ($a -gt $bestA) { $bestA = $a; $bottomFace = $f }
    }
}
if ($bottomFace) {
    $s7 = $cd.Sketches.Add($bottomFace)
    # FRONT view shows ONE small notch at center bottom of side profile.
    # FRONT = looking +Y direction. So notch is on -Y face (front face) of part.
    # On bottom face (z=0), notch sketch position: x=0 centered, y in [-37.5, -17.99] toward -Y edge.
    # Sketch face may flip; if rectangle covers wrong area, the notch ends up at +Y (back).
    # Width 7mm (x=±3.5), depth in z = 9mm. Length in Y = let's use small range.
    AddRect $s7 $tg -3.5 -38.0 3.5 -17.99
    $prof7 = $s7.Profiles.AddForSolid()
    $ed7 = $cd.Features.ExtrudeFeatures.CreateExtrudeDefinition($prof7, $kCut)
    $ed7.SetDistanceExtent((MM 9), $kNeg)
...
$out = "$env:USERPROFILE\Desktop\test\round2_EV-L200-MP_M-Right\my_attempt_v4.ipt"
