...
param(
    ...
    [double]$DowelX       = 9.0,     # 簣 from center
...
$f7.Name = "08_Keyway_W${KeyW}xD${KeyDepth}"
Write-Host "  ??$($f7.Name)"
...
if ($edgeCount -gt 0) {
    try {
        ...
        Write-Host "  Batch chamfer failed: $_  — falling back to per-edge" -ForegroundColor DarkYellow
        # Per-edge fallback
        $okCount = 0
        foreach ($e in $body.Edges) {
            if ($e.GeometryType -eq 5124) {
                try {
                    ...
                    $inFilletZ = [Math]::Abs($z - $hubBaseZ) -lt ($HubFilletR / 10.0 + 0.05)
                    $nearHubR  = ($r -gt ($hubBaseR - 0.05)) -and ($r -lt ($hubBaseR + $HubFilletR / 10.0 + 0.05))
                    if ($inFilletZ -and $nearHubR) { continue }
                    $tmp = $inv.TransientObjects.CreateEdgeCollection()
                    [void]$tmp.Add($e)
                    $fEach = $cd.Features.ChamferFeatures.AddUsingDistance($tmp, (MM $ChamfHole), $false)
                    $fEach.Name = "13_ChamferEdge_$okCount"
                    $okCount++
                } catch {}
            }
        }
        Write-Host "  ✓ Per-edge chamfer: $okCount succeeded" -ForegroundColor Green
    }
}
