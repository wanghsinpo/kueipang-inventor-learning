...
## 2026-05-10 — Computer-Use + Thumbnail + New Tools

### Computer-Use for Inventor 2027

**Discovery**: `mcp__computer-use__request_access` 必須在用戶在場時呼叫（需用戶點按鈕）；之後整個 session 免重問。

**Exact app name** (大小寫/語言精確):
```
Autodesk Inventor Professional 2027 - 简体中文 (Simplified Chinese)
```

**Flow**:
1. `request_access` (用戶在場 approve)
2. `open_application` → 等 splash screen 消失
3. 首次會出現「Autodesk 隱私聲明」對話框 → 點「我同意」後不再出現
4. `screenshot` → 可看到完整 Inventor UI

### View.SaveAsBitmap — 正確呼叫方式

**Signature**: `View.SaveAsBitmap(FileName, Width, Height)` — **3 個參數**
```powershell
$inv.ActiveView.SaveAsBitmap($thumbPath, 400, 300)
```
- 不需要 `InvokeMember`，直接 `$view.SaveAsBitmap(...)` 即可
- 無背景顏色參數（與 Inventor 2018 API 文件不同）
- 速度：~1.4秒/零件（含 Open + Fit + SaveAsBitmap + Close）

### SaveAsBitmap 找到的陷阱

- 5 個參數 → error（沒有 BackgroundColor 參數）
- 1 個參數 → error
- 3 個參數 → **SUCCESS**
- `$view.Fit()` 後加 `Start-Sleep -Milliseconds 150` 確保渲染完成再截圖

### auto_v4.ps1 — 通用幾何偵測

**判斷邏輯**: `cylFaces / totalFaces > 15%` AND `diam > thick * 0.5` → ring；否則 box
- 適用於 R1107-R1126 SKIP 資料夾（這些資料夾的 .ipt 以資料夾名命名，非 `real.ipt`）
- 先找 `real.ipt`，找不到就找 `*.ipt`（排除 `my_attempt*`）

### R1107-R1126 SKIP 原因釐清

這些資料夾的 .ipt 命名為 `<foldername>.ipt`，不是 `real.ipt`。
