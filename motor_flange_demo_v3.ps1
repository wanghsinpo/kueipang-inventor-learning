...
# ============================================================
# 11 — Chamfer all circular edges (hole entries, hub top, bore)
# ============================================================
Write-Host "[11] Chamfer all circular edges (C$ChamfHole)..." -ForegroundColor Yellow
$body = $cd.SurfaceBodies.Item(1)
$ecChamf = $inv.TransientObjects.CreateEdgeCollection()
$edgeCount = 0
foreach ($e in $body.Edges) {
    if ($e.GeometryType -eq 5124) {   # kCircleCurveObject (CLAUDE.md 坑 #3)
        [void]$ecChamf.Add($e)
        $edgeCount++
    }
}
if ($edgeCount -gt 0) {
    try {
        $f11 = $cd.Features.ChamferFeatures.AddUsingDistance($ecChamf, (MM $ChamfHole), $false)
        $f11.Name = "11_Chamfer_AllCircles_C${ChamfHole}"
        Write-Host "  ✓ $($f11.Name) — $edgeCount edges" -ForegroundColor Green
    } catch {
        Write-Host "  Chamfer failed: $_" -ForegroundColor DarkYellow
    }
} else {
    Write-Host "  No circular edges found" -ForegroundColor DarkYellow
}
