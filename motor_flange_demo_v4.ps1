...
# Find plate top + bottom face by area (largest planes with Z normal)
function Find-MaxPlane([double]$signZ) {
    $best = $null; $bestArea = 0
    foreach ($f in $body.Faces) {
        if ($f.SurfaceType -eq 5890) {
            $n = $f.Geometry.Normal
            if (($signZ -gt 0 -and $n.Z -gt 0.99) -or ($signZ -lt 0 -and $n.Z -lt -0.99)) {
                $area = $f.Evaluator.Area
                if ($area -gt $bestArea) { $bestArea = $area; $best = $f }
            }
        }
    }
    return $best
...
$plateTopFace = Find-MaxPlane 1
$plateBotFace = Find-MaxPlane -1
