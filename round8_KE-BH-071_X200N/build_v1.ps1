## Round 8 v1 — KE-BH-071 EV-X200N-MP-M 培林座
## NEW PATTERN: top slots and bottom slots are PERPENDICULAR (90° rotated):
##   - Top slots at ±X direction (walls at y=±4.5)
##   - Bottom slots at ±Y direction (walls at x=±3.5)
## This is different from Round 6 (KE-BH-070) where both pairs were aligned.
...
function Start-NagD([uint32]$invPid) {
...
"@
        ...
        while ((Get-Date) -lt $stop) {
            ...
            [ND]::EnumWindows({param($h,$l)
                $sb = New-Object Text.StringBuilder 256
                [void][ND]::GetWindowText($h, $sb, 256)
                [uint32]$wp=0; [void][ND]::GetWindowThreadProcessId($h, [ref]$wp)
                if ($wp -eq $p -and [ND]::IsWindowVisible($h) -and ($sb.ToString() -match 'Configurator|Sign In|Welcome')) { $script:hits.Add($h) }
                $true
            }, [IntPtr]::Zero) | Out-Null
            foreach ($h in $hits) { [void][ND]::PostMessage($h, 0x10, [IntPtr]::Zero, [IntPtr]::Zero) }
...
# Step 1: Pad Ø70 × 21.5
Write-Host "Step 1: Pad Ø70 × 21.5..." -ForegroundColor Cyan
...
$ed.SetDistanceExtent((MM 21.5), $kPos); $null = $cd.Features.ExtrudeFeatures.Add($ed)
$body = $cd.SurfaceBodies.Item(1)

# Step 2: Cavity Ø52 × 20 deep from top
Write-Host "Step 2: Ø52 cavity..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true; $s = $cd.Sketches.Add($tf)
...
$ed.SetDistanceExtent((MM 20), $kNeg); $null = $cd.Features.ExtrudeFeatures.Add($ed)

# Step 3: M5 thru at (0, ±30.5)
Write-Host "Step 3: M5 holes..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true; $s = $cd.Sketches.Add($tf)
$null = $s.SketchCircles.AddByCenterRadius($tg.CreatePoint2d((MM 0), (MM 30.5)), (MM 2.067))
$null = $s.SketchCircles.AddByCenterRadius($tg.CreatePoint2d((MM 0), (MM -30.5)), (MM 2.067))
$ed = $cd.Features.ExtrudeFeatures.CreateExtrudeDefinition($s.Profiles.AddForSolid(), $kCut)
$ed.SetDistanceExtent((MM 25), $kNeg); $null = $cd.Features.ExtrudeFeatures.Add($ed)

# Step 4: Ø5 thru at (±30.5, 0)
Write-Host "Step 4: Ø5 thru..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true; $s = $cd.Sketches.Add($tf)
...
# Step 5: Ø7.9 c'bores
Write-Host "Step 5: Ø7.9 c'bores..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true; $s = $cd.Sketches.Add($tf)
...
# Step 7: 2 BOTTOM slots at ±Y direction (walls x=±3.5)
Write-Host "Step 7: 2 BOTTOM slots at ±Y..." -ForegroundColor Cyan
...
# Step 8: 2 TOP slots at ±X direction (walls y=±4.5) — PERPENDICULAR to bottom!
Write-Host "Step 8: 2 TOP slots at ±X (perpendicular)..." -ForegroundColor Cyan
$tf = FindZ $body 21.5 $true
if ($tf) {
    $s = $cd.Sketches.Add($tf)
    # Top slot at +X: x in [26.5, 31], y in [-4.5, 4.5] (9mm wide in Y, 4.5mm long in X)
    AddRect $s $tg 26.5 -4.5 31.0 4.5
    AddRect $s $tg -31.0 -4.5 -26.5 4.5
    $ed = $cd.Features.ExtrudeFeatures.CreateExtrudeDefinition($s.Profiles.AddForSolid(), $kCut)
    $ed.SetDistanceExtent((MM 12.5), $kNeg); $null = $cd.Features.ExtrudeFeatures.Add($ed)
}
...
Write-Host ""
Write-Host ("BBox: {0} × {1} × {2}  (target 70 × 70 × 21.5)" -f $xL,$yL,$zL) -ForegroundColor Green
Write-Host ("Vol:  {0} mm³  (target 33,175 → diff {1:F2}%)" -f $vAct, ((($vAct - 33174.5)/33174.5)*100)) -ForegroundColor Green

$out = "$env:USERPROFILE\Desktop\test\round8_KE-BH-071_X200N\my_attempt.ipt"
